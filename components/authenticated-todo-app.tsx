...
export default function AuthenticatedTodoApp({ user }: { user: any }) {
  ...
  useEffect(() => {
    // 尝试从localStorage加载todos，使用用户ID作为键的一部分，以便每个用户有自己的todos
    const userKey = `todos_${user.id}`;
  ...
  useEffect(() => {
    // 保存todos到localStorage，使用用户ID作为键的一部分
    const userKey = `todos_${user.id}`;
